...
public class SequencerButtonToggle : MonoBehaviour
{
    ...
    [Header("Optional: Unique button ID")]
    public int buttonID = 0; // Set manually for each button if needed
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            handInside = true;
            Debug.Log($"Hand cube entered button {buttonID}.");
        }
    }
    ...
    private void OnTriggerExit(Collider other)
    {
        if (other.CompareTag("Hand"))
        {
            ...
            Debug.Log($"Hand cube exited button {buttonID}.");
        }
    }
    ...
    private void Update()
    {
        ...
        // Read the raw trigger value (0–1)
        ...
        if (triggerPressed && !triggerWasPressedLastFrame)
        {
            ...
            Debug.Log($"Button {buttonID} toggled → {toggleValue}");
        }
